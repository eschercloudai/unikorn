...
func New(options *Options, authenticator *authorization.Authenticator) (*Openstack, error) {
	...
	userID, err := os.ReadFile("/var/lib/secrets/unikorn.eschercloud.ai/keystone/userid")
	if err != nil {
		return nil, err
	}

	password, err := os.ReadFile("/var/lib/secrets/unikorn.eschercloud.ai/keystone/password")
	if err != nil {
		return nil, err
	}
...
func getProjectID(r *http.Request) (string, error) {
	...
	return claims.UnikornClaims.Project, nil
}

func isFederated(r *http.Request) (bool, error) {
	claims, err := oauth2.ClaimsFromContext(r.Context())
	if err != nil {
		return false, errors.OAuth2ServerError("failed get token claims").WithError(err)
	}

	if claims.UnikornClaims == nil {
		return false, errors.OAuth2ServerError("failed get token claim")
	}

	return claims.UnikornClaims.Federated, nil
}
...
func (o *Openstack) getApllicationCredentialClient(r *http.Request) (*openstack.IdentityClient, string, error) {
	federated, err := isFederated(r)
	if err != nil {
		return nil, "", err
	}
	...
	client, err := openstack.NewIdentityClient(openstack.NewBasicProvider(o.endpoint, o.userID, o.password, projectID))
	if err != nil {
		return nil, "", err
	}

	return client, o.userID, nil
}

func (o *Openstack) GetApplicationCredential(r *http.Request, name string) (*applicationcredentials.ApplicationCredential, error) {
	client, userID, err := o.getApllicationCredentialClient(r)
	if err != nil {
		return nil, err
	}

	result, err := client.ListApplicationCredentials(r.Context(), userID)
	if err != nil {
...
func (o *Openstack) CreateApplicationCredential(r *http.Request, name string, roles []string) (*applicationcredentials.ApplicationCredential, error) {
	...
	result, err := client.CreateApplicationCredential(r.Context(), userID, name, description, roles)
	if err != nil {
...
func (o *Openstack) DeleteApplicationCredential(r *http.Request, name string) error {
	...
	if err := client.DeleteApplicationCredential(r.Context(), userID, match.ID); err != nil {
