...
func (c *Checker) Check(ctx context.Context) error {
	...
	log.Info("checking for kubernetes cluster upgrades")
	...
	for _, resource := range resources.Items {
		if resource.Spec.ApplicationBundleAutoUpgrade == nil {
			log.Info("resource auto-upgrade disabled, ignoring", "project", resource.Labels[constants.ProjectLabel], "controlplane", resource.Labels[constants.ControlPlaneLabel], "cluster", resource.Name)
			continue
		}
		...
		if bundle.Spec.Preview != nil && *bundle.Spec.Preview {
			log.Info("bundle in preview, ignoring", "project", resource.Labels[constants.ProjectLabel], "controlplane", resource.Labels[constants.ControlPlaneLabel], "cluster", resource.Name)
			continue
		}

		// If the current bundle is the best option already, we are done.
		if bundle.Name == upgradeTarget.Name {
			log.Info("bundle already latest, ignoring", "project", resource.Labels[constants.ProjectLabel], "controlplane", resource.Labels[constants.ControlPlaneLabel], "cluster", resource.Name)
			continue
		}
